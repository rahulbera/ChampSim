...
$corr_timely_max 	= 20;
$corr_untimely_min 	= 0;
$corr_untimely_max 	= 15;
$incorr_min 		= -10;
$incorr_max 		= 0;
$none_min 		= -16;
$none_max 		= -2;
$out_of_bounds_min 	= -16;
$out_of_bounds_max 	= -2;
	...
	for(my $rew_corr_untimely = $corr_untimely_min; $rew_corr_untimely <= $corr_untimely_max; $rew_corr_untimely += 2)
	{
		for(my $rew_incorr = $incorr_min; $rew_incorr <= $incorr_max; $rew_incorr += 2)
		{
			for(my $rew_none = $none_min; $rew_none <= $none_max; $rew_none += 3)
			{
				...
				{
					print STDOUT "scooby_gs2_${rew_corr_timely}_${rew_corr_untimely}_${rew_incorr}_${rew_none}_${rew_out_of_bounds}   \$(BASE) \$(SCOOBY_CMAC) --scooby_reward_correct_timely=${rew_corr_timely} --scooby_reward_correct_untimely=${rew_corr_untimely} --scooby_reward_incorrect=${rew_incorr} --scooby_reward_none=${rew_none} --scooby_reward_out_of_bounds=${rew_out_of_bounds}\n";
					$total_exps++;
				}
			}
		}
