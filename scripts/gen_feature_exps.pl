...
my @selected_features = ("0", "10", "8", "13", "6");
my $num_comb = 3;
####################################################

my $num_tilings = join(",", (3)x$num_comb);
my $num_tlies = join(",", (128)x$num_comb);
...
foreach my $combo (@combinations)
{
	...
	$exp_name = "feat_sweep_tiny_${sel_feat_names}_3x128_hash2_no_dyn_weight_no_sel_update";
